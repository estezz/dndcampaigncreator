...
const generateSimpleText = async (prompt) => {
  ...
  if (result.promptFeedback) {
    ...
    console.warn(`Simple text generation blocked: ${blockReason}`, result); // Changed from console.error
  ...
  if (!text) {
    console.warn("Invalid response structure from generateSimpleText:", result); // Changed from console.error
...
const generateModuleText = async (formData, inspirationalImages) => {
  const { storyLine, otherThings, rewards, level, length, creativity, numPlayers, campaignSetting } = formData;
  ...
  const userPrompt = `
    ...
    **Setting (CRITICAL)**: All content (locations, themes, monsters, characters, items) MUST be appropriate for the **${campaignSetting}** D&D campaign setting.
  ...
  try {
    ...
    console.warn("Failed to parse JSON response:", parseError, result); // Changed from console.error
...
export default function App() {
  const [formData, setFormData] = useState({
    storyLinePrompt: "The isolated village of Mire’s End is slowly being consumed by a creeping, silent terror: every few nights, a villager vanishes without a cry, leaving behind only an unnerving trail of cold, black mud leading toward the surrounding brackish marshland. While the locals blame the encroaching bog-hags, the disappearances are the work of a far more sophisticated and sinister force—a subterranean cult of Kuo-Toa who have been driven mad by an abyssal artifact resting at the heart of their flooded grotto. The players must brave the treacherous, mist-choked wetlands, follow the disturbing, almost ritualistic signs of the abductions, and descend into the lightless, dripping caverns beneath the swamp to rescue the remaining captives before they are ritually sacrificed to summon a dreadful, tentacled horror from the depths.", // NEW
    storyLine: "The isolated village of Mire’s End is slowly being consumed by a creeping, silent terror: every few nights, a villager vanishes without a cry, leaving behind only an unnerving trail of cold, black mud leading toward the surrounding brackish marshland. While the locals blame the encroaching bog-hags, the disappearances are the work of a far more sophisticated and sinister force—a subterranean cult of Kuo-Toa who have been driven mad by an abyssal artifact resting at the heart of their flooded grotto. The players must brave the treacherous, mist-choked wetlands, follow the disturbing, almost ritualistic signs of the abductions, and descend into the lightless, dripping caverns beneath the swamp to rescue the remaining captives before they are ritually sacrificed to summon a dreadful, tentacled horror from the depths.", // This will be populated by the AI
    ...
    campaignSetting: "Forgotten Realms", // NEW: Added campaign setting
    level: 3,
    length: 4,
    creativity: 5,
    numPlayers: 4,
      ...
      <div className="md:flex">
        ...
        <aside className="w-full md:w-1/3 lg:w-1/4 p-4 md:p-6 bg-gray-900 md:bg-gray-800/50 md:sticky top-0 h-screen overflow-y-auto border-r border-gray-700/50">
          ...
          <form onSubmit={handleGenerateModule} className="space-y-4">
            ...
            {/* --- NEW CAMPAIGN SETTING DROPDOWN --- */}
            <div>
              <InputLabel htmlFor="campaignSetting">Campaign Setting</InputLabel>
              <select
                id="campaignSetting"
                ...
                className="block w-full bg-gray-800 border-gray-700 text-white rounded-md shadow-sm p-2 focus:ring-blue-500 focus:border-blue-500"
              >
                <option>Birthright</option>
                <option>Blackmoor</option>
                <option>Conan and Red Sonj</option>
                <option>Council of Wyrms</option>
                <option>Dark Sun</option>
                <option>Dragon Fist</option>
                <option>Eberron</option>
                <option>Forgotten Realms</option>
                <option>Kara-Tur</option>
                <option>Greyhawk</option>
                <option>Kingdoms of Kalamar</option>
                <option>Planescape</option>
                <option>Ravenloft</option>
                <option>Rokugan</option>
              </select>
            </div>
            {/* --- END OF NEW SECTION --- */}
